...
public:
	CCSDS(TypeCCSDS& packageCCSDS)
	{
		...
		for(size_t i = 0; i < 3; i++) this->version[i] = std::bitset<1>(packageID[i])[0] ;
		this->type = std::bitset<1>(packageID[3]);
		this->helperHeaderFlag = std::bitset<1>(packageID[4]);
		for(size_t i = 5; i < 16; i++) this->APID[i-5] = std::bitset<1>(packageID[i])[0];
		std::string packageControl = std::bitset<8>(packageCCSDS[2]).to_string() + std::bitset<8>(packageCCSDS[3]).to_string();
		for(size_t i = 0; i < 2; i++) this->fragmentationFlag[i] = std::bitset<1>(packageControl[i])[0];
		...
		for(size_t i = 14; i < size + 14; i++)
			this->payload.push_back(packageCCSDS[i]);
		this->countUsedByte = 14 + size; //14=len header CCSDS
	...
	unsigned int getToUlongHelpersHeaderFlag() { return this->helperHeaderFlag.to_ulong();}
...
public:
	...
	{	
		...
		/*
		this->offsetBit = std::bitset<16>(std::bitset<8>(packageVCDU[6]).to_string() + std::bitset<8>(packageVCDU[7]).to_string());
		
		size_t currentOffset = 6 + 2 + this->offsetBit.to_ulong();
		auto iterBegin = packageVCDU.begin();
		auto iterEnd = packageVCDU.end();
		unsigned int fragmentationFlag = 0;
		do
		{
			iterBegin += currentOffset;
			ParentCCSDS* parentCCSDS = new CCSDS<TypeVCDU>(TypeVCDU(iterBegin, iterEnd));
